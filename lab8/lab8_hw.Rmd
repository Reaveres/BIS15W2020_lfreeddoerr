...
write.fasta(names="covidseq", sequences=covidseq, file.out="covid_fasta_1.fasta")
covid <- read.fasta(file = "covid_fasta_1.fasta")
covidseq <- covid[[1]]
```
...
```{r}
covidsearch <- entrez_search(db = "nuccore", term = "SARS-CoV-2[ORGN]", retmax = 10)
```

```{r}
covidfetch <- entrez_fetch(db="nuccore", id=covidsearch$ids, rettype="fasta")
```

```{r}
write.fasta(names="covidseq", sequences="covidfetch", file.out="covid_seqs.fasta")
```
