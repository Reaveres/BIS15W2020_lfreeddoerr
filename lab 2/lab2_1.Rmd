...
```{r}
n_all_hp_matrix <- rbind(all_harry_potter_matrix, regional_earnings)
n_all_hp_matrix
```
...
```{r}
mean_vector <- rowMeans(spring_matrix)
mean_vector
```

3. Add `mean_vector` as a new column.  
```{r}
new_spring_matrix <- cbind(spring_matrix, mean_vector)
new_spring_matrix
```

Commands I learned: 
harry_potter_matrix <- matrix(box_office, nrow = 8, byrow = T)
colnames(harry_potter_matrix) <- region                          (also rownames)
global <- rowSums(harry_potter_matrix)                           (also colSums)
all_harry_potter_matrix <- cbind(harry_potter_matrix, global)    (also rbind)
mean_vector <- rowMeans(spring_matrix)                           (colMeans also exists)
