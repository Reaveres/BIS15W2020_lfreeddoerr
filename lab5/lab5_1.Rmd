...
You could treat it as an actual value, or not use NA functions....
...
```{r}
import_export %>%
  summarize(na_im_ex = sum(is.na(import_export)))
```
...
Yes, there are very likely NAs represented as a negative value like -999--we can tell because numbers that should only be positive, like max_life and litters_yr, are negative.
