...
```{r}
ui <- fluidPage(
    radioButtons("x", "Select X Variable", choices = c("Sepal.Length", "Sepal.Width", "Petal.Length", "Petal.Width"), 
              selected = "Petal.Width"),
    selectInput("y", "Select Y Variable", choices = c("Sepal.Length", "Sepal.Width", "Petal.Length", "Petal.Width"), 
              selected = "Petal.Length")
)
...
ui <- fluidPage(    
  ...
  titlePanel("Log 10 Homerange by Both Taxon and Locomotion"), # give the page a title
  ...
  sidebarLayout(      
    ...
    # define the sidebar with one input
    sidebarPanel(
      selectInput("taxon_locomotion", " Select Taxon or Locomotion of Interest:", 
                  choices=c(unique(homerange$taxon), unique(homerange$locomotion))),
      hr(),
      helpText("Reference: Tamburello N, Cote IM, Dulvy NK (2015) Energy and the scaling of animal space use. The American Naturalist 186(2):196-211.")
    ),
...
server <- function(input, output) {
  ...
  output$bothPlot <- renderPlot({
    ...
    homerange %>% 
    filter(locomotion == input$locomotion | taxon == input$taxon) %>% 
    ggplot(aes(x=log10.hra)) + 
    geom_density(color="black", fill="steelblue", alpha=0.6)
  })
}
